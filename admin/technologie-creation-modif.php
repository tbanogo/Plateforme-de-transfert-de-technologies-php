...
if(!empty($_POST)){
    ...
    if(file_exists($chemin_document_a_supprimer)) unlink($chemin_document_a_supprimer);
    //fin
    executeRequete("DELETE FROM technologie WHERE id_techn=$_GET[action];");
    executeRequete("INSERT INTO technologie (id_techn, designation, auteurs, cultures, documents, villes, date_enre, date_publication, publicateur) values ('$_GET[action]', '$_POST[designation]', '$auteur_total', '$culture', '$document', '$villes', '$data', '$_POST[date_mis]', '$ses')");
                        ...
                        <div class="col-md-6">
                            <div class="form-group my-5">
                                <div class="input-group input-group-lg">
                                    ...
                                    <input id="designation" name="designation" class="form-control" value="<?= $technologie['designation']; ?>" type="text" required="required">
                                </div>
                            </div>
                            <div class="form-group my-5">
                                <div class="input-group input-group-lg">
                                    ...
                                    <input id="document" name="document" class="form-control" id="file-control-" type="file" accept="image/*,video/*,.pdf" required="required" multiple>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group my-5">
                                <div class="input-group input-group-lg">
                                    ...
                                    <?php
                                        $chaine = explode(', ', $technologie['cultures']);
                                    ?>
                                    ...
                                    <?php
                                        if($chaine[0] == 'Mustard')
                                            echo '<option selected>Mustard</option>';
                                        else
                                            echo '<option>Mustard</option>';

                                        if($chaine[0] == 'Ketchup' || $chaine[1] == 'Ketchup')
                                            echo '<option selected>Ketchup</option>';
                                        else
                                            echo '<option>Ketchup</option>';

                                        if($chaine[0] == 'Relish' || $chaine[1] == 'Relish' || $chaine[2] == 'Relish')
                                            echo '<option selected>Relish</option>';
                                        else
                                            echo '<option>Relish</option>';
                                    ?>
                                    </select>
                            ...
                            <div class="form-group my-5">
                                <div class="input-group input-group-lg">
                                    ...
                                    <?php
                                        $chaine_villes = explode(', ', $technologie['villes']);
                                    ?>
                                    <select class="selectpicker show-menu-arrow form-control form-control-lg" data-width="auto" id="villes" name="villes[]" multiple>
                                        <?php
                                        if($chaine_villes[0] == 'Ouagadougou')
                                            echo '<option selected>Ouagadougou</option>';
                                        else
                                            echo '<option>Ouagadougou</option>';

                                        if($chaine_villes[0] == 'Bobo Dioulasso' || $chaine_villes[1] == 'Bobo Dioulasso')
                                            echo '<option selected>Bobo Dioulasso</option>';
                                        else
                                            echo '<option>Bobo Dioulasso</option>';

                                        if($chaine_villes[0] == 'Banfora' || $chaine_villes[1] == 'Banfora' || $chaine_villes[2] == 'Banfora')
                                            echo '<option selected>Banfora</option>';
                                        else
                                            echo '<option>Banfora</option>';

                                        if($chaine_villes[0] == 'Koudougou' || $chaine_villes[1] == 'Koudougou' || $chaine_villes[2] == 'Koudougou'|| $chaine_villes[3] == 'Koudougou')
                                            echo '<option selected>Koudougou</option>';
                                        else
                                            echo '<option>Koudougou</option>';
                                    ?>
                                    </select>
                                    <div class="input-group-append">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="date" name="date_mis" id="date_mis" value="<?= $technologie['date_publication'] ?>" class="form-control" required="required">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="input-group input-group-lg">
                                    ...
                                    <input name="auteurs" id="auteurs" class="form-control w-25" value="<?= $technologie['auteurs'] ?>" type="text" required="required">
                    ...
                    <div class="row">
                        <div class="form-group mx-auto">
                            <div class="input-group">
                                <button type="submit" class="btn btn-primary" style="width: 200px;" btn-block>Modifier</button>
                            </div>
                        </div>
                    </div>
                </div> 
